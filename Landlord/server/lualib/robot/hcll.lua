...
function H:hcll_init(msg)
	log:debug("横财来了： 免费次数 %d,基础分 %d", msg.free, msg.curr)
	for i,v in ipairs(msg.level) do
		log:debug("横财来了低分档次：%d档 %d分", i, v)
	end

	local idx = math.random( 1, #msg.level)
	client.push(self,'hcll_set',{idx=idx})
	client.push(self,'hcll_play',{})

end

function H:hcll_result(msg)
    local str = ""
