...
local function query_addr(channel,node,name)
	local session = node_session[node] or 1
	-- msg is a local pointer, cluster.packrequest will free it
	local request, new_session, padding = cluster.packrequest(0, session, skynet.pack(name))
	node_session[node] = new_session
	local addr=skynet.unpack(assert(channel:request(request, session, padding)))
	return addr
end
...
function command.getconfig()
	return skynet.retpack(node_address)
end
...
local function get_nodeaddr(node,addr)
	-- node_channel[node] may yield or throw error
	local c = node_channel[node]
	local taddr=c.addrcache[addr]
	if not taddr then
		taddr=query_addr(c,node,addr)
		c.addrcache[addr]=taddr
	end
	return c,taddr
end

local function send_request(source, node, addr, msg, sz)
	local c,taddr=get_nodeaddr(node,addr)
	
	-- msg is a local pointer, cluster.packrequest will free it
	local session = node_session[node] or 1
	local request, new_session, padding = cluster.packrequest(taddr, session, msg, sz)
	node_session[node] = new_session
...
function command.push(source, node, addr, msg, sz)
	local session = node_session[node] or 1
	local c,taddr=get_nodeaddr(node,addr)
	local request, new_session, padding = cluster.packpush(taddr, session, msg, sz)
...
local register_name = {}
register_name[skynet.self()] = "clusterd"
register_name["clusterd"] = skynet.self()
